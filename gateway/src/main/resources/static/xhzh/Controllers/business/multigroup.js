...
xh.load = function() {
	...
	var cMultigroupid = $("#cMultigroupid").val();
	var eName = $("#eName").val();
	var pageSize = $("#page-limit").val();

	app.controller("multigroup", function($scope, $http) {
		xh.maskShow();
		$scope.count = "20";// 每页数据显示默认值
		$scope.systemMenu = true; // 菜单变色
		$http.get(
				"../../multigroup/list?cMultigroupid=" + cMultigroupid + "&eName=" + eName
		...
		$scope.search = function(page) {
			...
			xh.maskShow();
			$http.get(
					"../../multigroup/list?cMultigroupid=" + cMultigroupid + "&eName=" + eName + "&start="
...
// 刷新数据
xh.refresh = function() {
	var appElement = document.querySelector('[ng-controller=multigroup]');
