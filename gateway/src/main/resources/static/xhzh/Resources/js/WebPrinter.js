...
(function (win) {
    var PRINTER = {
        strHostUrl: "http://localhost:59418/",
        DoPostDatas: function (funName) {
            var szResponse
            var szUrl = this.strHostUrl + funName;
            var numargs = arguments.length;
            for (i = 1; i < numargs; i++) {
                szUrl += (i > 1) ? "&" : "?";
                szUrl += "p" + i.toString() + "=" + arguments[i];
            }
            var oXMLHttpRequest = new XMLHttpRequest;
            oXMLHttpRequest.open("POST", szUrl, false);
            oXMLHttpRequest.onreadystatechange = function () {
                if (this.readyState == XMLHttpRequest.DONE) {
                    szResponse = this.responseText;
                }
            }
        ...
        GET_PRINTER_COUNT: function () {
            try {
                return parseInt(this.DoPostDatas("GET_PRINTER_COUNT"));
            } catch (err) { }
            return 0;
        },
        GET_PRINTER_NAME: function () {
            var intNO = arguments[++i] ? arguments[i] : -1;
            return this.DoPostDatas("GET_PRINTER_NAME", intNO);
        },
        PRINT_INIT: function () {
            this.DoPostDatas("PRINT_INIT");
        },
        SET_PRINTER_INDEX: function (intNO) {
            this.DoPostDatas("SET_PRINTER_INDEX", intNO);
        },
        SET_PRINT_COPIES: function (intCopies) {
            this.DoPostDatas("SET_PRINT_COPIES", intCopies);
        },
        SET_PAPER_SIZE: function (w, h) {
            this.DoPostDatas("SET_PAPER_SIZE", w, h);
        },
        SET_PAPER_ORIENT: function (intOrient) {
            this.DoPostDatas("SET_PAPER_ORIENT", intOrient);
        },
        SET_PRINT_STYLE: function (name, value) {
            this.DoPostDatas("SET_PRINT_STYLE", name, value);
        },
        ADD_PRINT_TEXT: function (x, y, w, h, strText) {
            this.DoPostDatas("ADD_PRINT_TEXT", x, y, w, h, encodeURIComponent(strText));
        },
        ADD_PRINT_HTML: function (x, y, w, h, strHtml) {
            this.DoPostDatas("ADD_PRINT_HTML", x, y, w, h, strHtml);
        },
        NEW_PAGE: function () {
            this.DoPostDatas("NEW_PAGE");
        },
        PREVIEW: function () {
            this.DoPostDatas("PREVIEW");
        }
    };
    win.PRINTER = PRINTER;
})(window);
